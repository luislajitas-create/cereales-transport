...
// CAT-2: mismo criterio de mock que clientes.controller.importar.spec.ts / transportistas.
// controller.importar.spec.ts (CAT-1) — sin DB real. La resolución de transportista por CUIT y
// la detección de duplicados por CUIL/DNI se mockean como las dos únicas consultas en lote que
// hace el controller (transportista.findMany/chofer.findMany), nunca una por fila. El aislamiento
// por organización (Bloque 8.1.d) es transparente acá: en producción, transportista.findMany ya
// viene acotado a la organización activa — por eso "transportista de otra organización" se
// simula exactamente igual que "transportista inexistente" (el mock simplemente no lo incluye
// en el resultado), tal como lo haría la extensión real.
...
describe("ChoferesController.importar (CAT-2)", () => {
  ...
  it("archivo válido: crea todas las filas y devuelve el resumen correcto", async () => {
    ...
    const archivo = crearArchivo(
      "transportistaCuit,nombre,dni,cuil,comisionPct,licenciaNumero,licenciaVencimiento,telefono\n" +
        "30-11111111-1,Juan Perez,30123456,20-30123456-4,5,B123,,\n" +
  ...
  it("CUIL duplicado en base: la fila se rechaza y no se llama a create", async () => {
    const { prisma, crear } = crearPrismaMock({ choferesExistentes: [{ cuil: "20-30123456-4" }] });
    const controller = new ChoferesController(prisma);
    ...
    expect(resultado.rechazados).toBe(1);
    expect(resultado.detalle[0].mensaje).toContain("Ya existe un chofer con CUIL '20-30123456-4'");
    expect(crear).not.toHaveBeenCalled();
  });

  it("CUIL duplicado dentro del archivo: solo la primera ocurrencia se crea, la segunda se rechaza", async () => {
    const { prisma, crear } = crearPrismaMock();
    const controller = new ChoferesController(prisma);
    const archivo = crearArchivo(
      "transportistaCuit,nombre,cuil\n30-11111111-1,Juan Perez,20-30123456-4\n30-11111111-1,Juan Perez Duplicado,20-30123456-4",
    );
  ...
  it("DNI duplicado dentro del archivo: solo la primera ocurrencia se crea, la segunda se rechaza", async () => {
    const { prisma, crear } = crearPrismaMock();
    const controller = new ChoferesController(prisma);
    const archivo = crearArchivo(
      "transportistaCuit,nombre,dni,cuil\n" +
        "30-11111111-1,Juan Perez,30111222,20-10000000-1\n" +
  ...
  it("DNI vacío en varias filas: no se consideran duplicadas entre sí (DNI es opcional)", async () => {
    const { prisma, crear } = crearPrismaMock();
    const controller = new ChoferesController(prisma);
    const archivo = crearArchivo(
  ...
  it("límite de filas: permite exactamente el límite configurado", async () => {
    const { prisma, crear } = crearPrismaMock();
    const controller = new ChoferesController(prisma);
    const filas = Array.from({ length: LIMITE_FILAS_IMPORTACION_CSV }, (_, i) => `30-11111111-1,Chofer ${i},,CUIL-${i},,,,`);
    const archivo = crearArchivo(
      "transportistaCuit,nombre,dni,cuil,comisionPct,licenciaNumero,licenciaVencimiento,telefono\n" + filas.join("\n"),
    );
  ...
  it("límite de filas: rechaza el archivo completo si lo supera en una sola fila, sin crear nada", async () => {
    ...
    const controller = new ChoferesController(prisma);
    const filas = Array.from({ length: LIMITE_FILAS_IMPORTACION_CSV + 1 }, (_, i) => `30-11111111-1,Chofer ${i},,CUIL-${i},,,,`);
    const archivo = crearArchivo(
      "transportistaCuit,nombre,dni,cuil,comisionPct,licenciaNumero,licenciaVencimiento,telefono\n" + filas.join("\n"),
    );
